...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        trap_temp.set_ion(Sr88Ion(), Field2D(side / 2 + 0.4e-3, side / 2 + 0.3e-3), Field2D(400, 400))
    ...
    trap_with_pot.set_bias([0, v_rf, 0, v_rf])

    trap = Trap2D(trap_with_pot)
    trap.set_simulation_parameters(100, 100)
    trap.set_ion(Sr88Ion(), Field2D(side / 2 + 0.4e-3, side / 2 + 0.3e-3), Field2D(400, 400))
    trap.start_simulation(False)

    results = trap.results
    t = map(lambda x: x[0] * 1000000, results)
    x = map(lambda x: x[1].x, results)
    y = map(lambda x: x[1].y, results)
